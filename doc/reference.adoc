...
collection of objects.  Sets are the primary data structure in CCMS;
however, query commands such as `select` or `show sets` return a list.
...
select isbn from s tag t
----

Multiple tags can be listed, separated by commas:

----
select isbn from s tag t, u, not v, w
----
...
it, the `select dynamic` command can be used to request values for one
or more dynamic attributes in a single record.  This form of query
...
----
{
  "command": "select dynamic cost from myset where id = \"053e4063-d70c-4a41-a968-e6900e6b47d5\""
}
----
...
which to retrieve data, or `*` meaning all available attributes.
...
----
select id, title from reserve limit 5
----
